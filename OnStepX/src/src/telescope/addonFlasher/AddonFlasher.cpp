...
#if SERIAL_INPUT == HardSerial
#define SERIAL_INPUT_RXTX_SET
extern HardwareSerial HWSerialC;
#undef SERIAL_INPUT
#define SERIAL_INPUT HWSerialC
#endif
...
// ADDON_GPIO0_PIN  HIGH for run and LOW for firmware upload
  ...
  void AddonFlasher::go(bool timeout) {
    ...
    while (true) {
      ...
      if (SERIAL_PASSTHROUGH.available()) {
        ...
        delayMicroseconds(5);
        SERIAL_INPUT.write(inByte);
        delayMicroseconds(5);
      }

      // read from port 0, send to port 1:
      if (SERIAL_INPUT.available()) {
        int inByte = SERIAL_INPUT.read();
        delayMicroseconds(5);
  ...
  void AddonFlasher::run(bool setSerial) {
    ...
    if (setSerial) {
      ...
      #if defined(SERIAL_INPUT_RX) && defined(SERIAL_INPUT_TX) && !defined(SERIAL_INPUT_RXTX_SET)
        SERIAL_INPUT.begin(SERIAL_INPUT_BAUD_DEFAULT, SERIAL_8N1, SERIAL_INPUT_RX, SERIAL_INPUT_TX);
      #else
        SERIAL_INPUT.begin(SERIAL_INPUT_BAUD_DEFAULT);
      #endif
      tasks.yield(1000);
    }
  }
  ...
  void AddonFlasher::flash() {
    ...
    #if defined(SERIAL_INPUT_RX) && defined(SERIAL_INPUT_TX) && !defined(SERIAL_INPUT_RXTX_SET)
      SERIAL_INPUT.begin(115200, SERIAL_8N1, SERIAL_INPUT_RX, SERIAL_INPUT_TX);
    #else
      SERIAL_INPUT.begin(115200);
    #endif
